...
export default {
    ...
    data() {
        return {
            ...
            filename: 'horizontal_project_info'
        }
    },
    created() {
    ...
    methods: {
        ...
        async handleDelete(index, row) {
            ...
            let res = await http.post("/hproject/delete/" + row.projectId);
            if (res && res.code == 200) {
        ...
        doSubmit() {
            ...
            this.$refs['project'].validate(async (valid) => {
                if (valid) {
                    ...
                    //获取后台请求书籍数据的地址
                    let res = await http.post("/hproject/upsert", this.project);
                    console.log(res);
                    if (res && res.code == 200) {
        ...
        async fetch() {
            ...
            let res = await http.get("/hproject/list");
            if (res && res.code == 200) {
        ...
        async doSearch() {
            ...
            let res = await http.get("/hproject/search", this.searchProject);
            if (res && res.code == 200) {
                console.log(res.data);
